...
from utils.data_preprocess import find_weights_norm
...
class batch_localization_case():
    def __init__(self, batch_size, cams, cams_id, init_loc, target_pix=None):
    ...
    def solve_frame_loc(self,rho,na):
        def get_obs(na):
            obs = []
            for j,id in enumerate(self.cams_id[0]):
                cam = self.cams[id]
                anchor_loc = cam.anchors[0:na]
                prox_anchor_pix = np.array(cam.prox_anchor_pix)[0:na]
                gt_anchor_pix = np.array(cam.gt_anchor_pix)[0:na]
                obs_pix = self.target_pix[0][j]
                # w = find_weights_pos(init_loc,anchor_loc,ord=2)
                w = find_weights_norm(self.start,anchor_loc,ord=2)
                w = np.array(w).reshape(1,len(w))
                anchor_term = np.dot(w,(prox_anchor_pix-gt_anchor_pix)).squeeze()
                obs.append(obs_pix + anchor_term)
            obs = np.array(obs)
            self.obs = obs
        def loss_function(z):
            res = 0
            ...
            for i,id in enumerate(self.cams_id[0]):
                ...
                z_pix = np.array(z_pix)
                res += np.linalg.norm(z_pix-self.obs[i])
            return res
        get_obs(na)
