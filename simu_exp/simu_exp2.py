    ...
    for id in ids:
        ...
        for t in range(config.t1, config.t2-config.batch_size):
            ...
            result_data.append({
                ...
                "frame_loc_10": frame_locs[1],
                "no_anchor_loc": no_anchor_loc,
                "no_anchor_gt_loc": no_anchor_gt_loc
            })
    result_df = pd.DataFrame(result_data)

    error_combine = str(config.rx_error) + '_' + str(config.ry_error) + '_' + str(config.rz_error) + '_' + str(config.T_error) + '_' + str(config.K_error) + '_' + str(config.D_error) + '_' + str(config.pix_perturb_level)

    with open(f'exp_result/exp5/{error_combine}.pkl', 'wb') as file:
        pickle.dump(result_df, file)
    
    print('Experiment for Fig.5')
